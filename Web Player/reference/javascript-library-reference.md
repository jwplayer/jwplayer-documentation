...
- For an introduction to JW8's API, see our [Introduction to the JavaScript API](//developer.jwplayer.com/jw-player/docs/developer-guide/api/javascript_api_introduction/). 
- For more in-depth instructions on implementation, see our [JW Player Developer Guide](//developer.jwplayer.com/jw-player/docs/developer-guide/). 
- To see example code for each API call, visit our [JW Player Demos](//developer.jwplayer.com/jw-player/demos/) and search by call.
...
!!!tip
To view the order of player events for a player on your site, you can install <a href="https://chrome.google.com/webstore/detail/jw-lens/oneljddokkbioaemddedcbdejocnhijg" target="_blank">JW Lens</a>, a Chrome browser extension, and follow these <a href="https://developer.jwplayer.com/jw-player/docs/developer-guide/troubleshooting/jw-lens-overview/#events" target="_blank">steps to view the event order</a>.
!!! 
...
Fired when the player enters the playing state<br/><br/>
The play event fires when non-advertising media within the player begins playback. To listen for an ad break play event, use [jwplayer().on('adPlay')](#adplay).
...
Fired when the player enters the paused state<br/><br/>
The pause event fires when non-advertising media within the player is paused. To listen for an ad break pause event, use [jwplayer().on('adPause')](#adpause).
...
|Attribute | Description | Type | Required |
| -- | -- | -- | -- |
| begin | Cue location relative to the start of the media item, in seconds | Number | Yes |
| text | Label for the cue | String | Yes |
...
### jwplayer().on('adManager') <sup>< 8.8.0 </sup>

**DEPRECATED**: Starting with JW Player 8.8.0, use [jwplayer().on('adsManager')](#adsmanager).
...
|Value|Description|Type|
|--|--|--|
|`adManager`|Object containing the ad manager configuration|Object|
|`type`|Category of the player event<br/><br/>This is always `adManager` for this event.|String|
...
<a name="adsmanager"></a>
### jwplayer().on('adsManager') <sup>8.5.2+ (IMA) / 8.8.0+ (FreeWheel, IMA) </sup>
...
|Value|Description|Type|
|--|--|--|
|`adsManager`|Object containing the ad manager configuration<br/><br/>See <a href="https://developers.google.com/interactive-media-ads/docs/sdks/html5/v3/apis#ima.AdsManager" target="_blank">Google's</a> and <a href="https://hub.freewheel.tv/api_docs/html5" target="_blank">FreeWheel's</a> documentation for the properties returned by the respective SDK.|Object|
|`type`|Category of the player event<br/><br/>This is always `adsManager` for this event.|String|
|`videoElement`|(IMA only) Video tag used for playback|Object|
