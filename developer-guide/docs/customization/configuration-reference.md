...
| `placement` | Number | Value sent in a bid request that identifies the location of a player<br /><br />This value provides advertisers with more information before bidding for a video ad opportunity.<br /><br />You can set one fo the following possible values include:<br /><br />&bull; `1`: instream<br />&bull; `2`: banner<br />&bull; `3`: article<br />&bull; `4`: feed<br />&bull; `5`: floating \| interstitial \| slider| IMA, VAST|`3` (article) when `advertising.outstream: true`<br /><br />`1` (instream) for all other configurations | 
