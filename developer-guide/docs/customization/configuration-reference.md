...
|--|--|--|--|
| `pauseAds` <sup>8.10.0+</sup> | Boolean | Controls if ad playback stops when the player is no longer viewable.<br /><br />&bull; `true`: When the player is no longer viewable, ad playback pauses. Ad playback resumes when the player becomes viewable again.<br /><br />&bull; `false`: Only video playback will be paused when the player is no longer viewable. <br /><br />**NOTE**: If `viewability: false`, setting `pauseAds: true` will have no effect. | `false` |
