...
The `message` property contains the viewer-friendly text displayed on the player's error screen. Error messages are localizable. Visit the [localization reference](../customization/configuration-reference/#localization) for more information.
...
|<a name='224005'></a> 224005 | The video tag `src` was set to empty (`""`) which resolved to the current page url.  | - |This video cannot be played because of a technical error.|
