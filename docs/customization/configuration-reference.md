...
|**defaultBandwidthEstimate** <sup>8.3+</sup>|Number|Sets an initial bandwidth estimate in bits for all viewers. All values, including negative ones, below the minimum of 1 will be rounded up, whereas  `Math.max()` can be used to set the initial estimate to the highest possible.|-|
